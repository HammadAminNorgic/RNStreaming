...
struct REQUESTED_ADDRESS_FAMILY: Attribute {
    let protocolFamily: ProtocolFamily
    let type = AttributeType.CHANGE_REQUEST
    let length = 8
    ...
    func toData() -> [UInt8] {
        let bodyData = [UInt8(type.rawValue / 256),
...
struct CHANGE_REQUEST: Attribute {
    let protocolFamily: ProtocolFamily
    let type = AttributeType.CHANGE_REQUEST
    let length = 4
    let changeType: ChangeType
    
    init(protocolFamily: ProtocolFamily, changeType: ChangeType) {
