...
export default class RTCPeerConnection extends defineCustomEventTarget(...PEER_CONNECTION_EVENTS) {
    ...
    constructor(configuration) {
        ...
        super();
        console.log('this is configs-->',configuration)
        this._pcId = nextPeerConnectionId++;
        // console.log('this is configs-->',VdotokStreamingModule)

        VdotokStreamingModule.peerConnectionInit(configuration, this._pcId);
// return
    ...
    createOffer(options) {
        ...
        return new Promise((resolve, reject) => {
            VdotokStreamingModule.peerConnectionCreateOffer(
                this._pcId,
    ...
    createAnswer() {
        ...
        return new Promise((resolve, reject) => {
            VdotokStreamingModule.peerConnectionCreateAnswer(
                this._pcId,
                {},
                (successful, data) => {
                    if (successful) {
    ...
    setConfiguration(configuration): void {
        VdotokStreamingModule.peerConnectionSetConfiguration(configuration, this._pcId);
    }
    ...
    async setLocalDescription(sessionDescription?: RTCSessionDescription | RTCSessionDescriptionInit): Promise<void> {
        ...
        const {
            sdpInfo,
            transceiversInfo
        } = await VdotokStreamingModule.peerConnectionSetLocalDescription(this._pcId, desc);
    ...
    setRemoteDescription(sessionDescription: RTCSessionDescription | RTCSessionDescriptionInit): Promise<void> {
        ...
        return new Promise((resolve, reject) => {
            VdotokStreamingModule.peerConnectionSetRemoteDescription(
                desc,
                this._pcId,
                (successful, data) => {
                    if (successful) {
    ...
    async addIceCandidate(candidate): Promise<void> {
        ...
        const newSdp = await VdotokStreamingModule.peerConnectionAddICECandidate(
            this._pcId,
    ...
    addTrack(track: MediaStreamTrack, ...streams: MediaStream[]): RTCRtpSender {
        ...
        const result = VdotokStreamingModule.peerConnectionAddTrack(this._pcId, track.id, { streamIds });
    ...
    addTransceiver(source: 'audio' | 'video' | MediaStreamTrack, init): RTCRtpTransceiver {
        ...
        const result = VdotokStreamingModule.peerConnectionAddTransceiver(this._pcId, { ...src, init: { ...init } });
    ...
    removeTrack(sender: RTCRtpSender) {
        ...
        // Blocking!
        VdotokStreamingModule.peerConnectionRemoveTrack(this._pcId, sender.id);
    ...
    getStats() {
        ...
        return VdotokStreamingModule.peerConnectionGetStats(this._pcId).then(data =>
    ...
    close(): void {
        ...
        VdotokStreamingModule.peerConnectionClose(this._pcId);
    ...
    restartIce(): void {
        VdotokStreamingModule.peerConnectionRestartIce(this._pcId);
    }
    ...
    createDataChannel(label: string, dataChannelDict?: RTCDataChannelInit): RTCDataChannel {
        ...
        const channelInfo = VdotokStreamingModule.createDataChannel(this._pcId, label, dataChannelDict);
