...
export default class RTCRtpSender {
    ...
    async replaceTrack(track: MediaStreamTrack | null): Promise<void> {
        try {
            await VdotokStreamingModule.senderReplaceTrack(this._peerConnectionId, this._id, track ? track.id : null);
        } catch (e) {
            return;
        }
    ...
    async setParameters(parameters: RTCRtpSendParameters): Promise<void> {
        ...
        const newParameters = await VdotokStreamingModule.senderSetParameters(this._peerConnectionId, this._id, _params);
